...
const stripAutoDefaultServiceBundle = (services = []) => {
  const serviceList = (Array.isArray(services) ? services : [])
    ...
    .filter(Boolean);
  const serviceKeys = new Set(serviceList.map((service) => getServiceKey(service)).filter(Boolean));
  ...
  return serviceList.filter((service) => !AUTO_DEFAULT_SERVICE_KEYS.has(getServiceKey(service)));
};
...
const getItemServices = (item = {}) => stripAutoDefaultServiceBundle([
  ...new Set(
    [
