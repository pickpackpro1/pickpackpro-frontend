...
const buildBundleSizeNote = (items = []) => {
  const bundleSizes = (Array.isArray(items) ? items : [])
    .map((item) => ({
      ...
      hasBundling: Boolean(item?.needsBundling || item?.needs_bundling || (item?.services || []).some(isBundlingServiceValue)),
    }))
    .filter((item) => item.hasBundling && isPositiveBundleSize(item.bundleSize) && (item.sku || item.fnsku))
    .map(({ hasBundling, ...item }) => item);

  return bundleSizes.length ? `${BUNDLE_SIZE_NOTE_PREFIX} ${JSON.stringify(bundleSizes)}` : '';
};
...
const applyBundleSizesFromNotes = (items = [], shipment = {}) => {
  ...
  return (Array.isArray(items) ? items : []).map((item) => {
    ...
    return isPositiveBundleSize(bundleSize)
      ? { ...item, bundleSize, bundle_size: bundleSize, needsBundling: true, needs_bundling: true }
      : item;
  });
};
...
const getLineItemNaturalDisplayOrder = (item = {}) =>
  parseLineItemSequenceOrder(getLineItemProductName(item), ['product', 'prod', 'pp', 'p']) ??
  parseLineItemSequenceOrder(getLineItemSku(item), ['sku', 'product', 'prod', 'pp', 'p']) ??
  parseLineItemSequenceOrder(getLineItemFnsku(item), ['fnsku', 'f', 'sku', 'product', 'prod', 'pp', 'p']);
...
const buildShipmentNotes = (notes = '', items = []) =>
  [String(notes || '').trim(), buildBundleSizeNote(items)].filter(Boolean).join('\n');
...
const getBoxStatus = (box = {}) =>
  firstPresent(box?.status, box?.boxStatus, box?.box_status, box?.state, box?.dispatchStatus, box?.dispatch_status, box?.labelStatus, box?.label_status) || 'No status';
...
const rawFileMatchesLineItem = (file = {}, item = {}) => {
  ...
  const itemSku = normalizeItemFileMatchValue(getLineItemSku(item));
  const itemFnsku = normalizeItemFileMatchValue(getLineItemFnsku(item));
  ...
  return Boolean(
    (itemLabelFileId && fileRecordId && itemLabelFileId === fileRecordId) ||
      (entityId && itemIds.includes(entityId)) ||
      (itemSku && fileSku && itemSku === fileSku) ||
      (itemFnsku && fileFnsku && itemFnsku === fileFnsku) ||
...
const decorateItemLabelFile = (file = {}, item = {}) => {
  const itemId = getLineItemRecordId(item);
...
const getLineItemDisplayProductName = (item = {}, labelFile = null) => {
  ...
  if (isAutoGeneratedProductName(productName)) {
    return getAutoGeneratedProductAlias(productName) || productName;
  }
...
const findLineItemLabelFile = (item = {}, files = [], itemCount = 0, index = 0) => {
  const fileList = getDisplayableItemFiles(mergeFileLists(getItemInlineLabelFiles(item), files));
  if (!fileList.length) return null;

  const fnskuLabelFiles = fileList.filter(isFnskuLabelFile);
  const labelFiles = fileList.filter(isAnyItemLabelFile);
  const pdfFiles = fileList.filter(isPdfFile);
  const imageFiles = fileList.filter(isImageFile);
  const csvFiles = fileList.filter(isCsvFile);
  const matchingFiles = fileList.filter((file) => fileMatchesLineItem(file, item));
  const matchingFile =
...
const isItemLabelCandidateFile = (file = {}, itemList = []) => {
  ...
  if (getFileItemIndex(file) !== null) return isSupportedItemLabelFile(file) && (isFnskuLabelFile(file) || isAnyItemLabelFile(file));
...
const getLineItemFiles = (item = {}, files = [], itemCount = 0, index = 0) => {
  const fileList = getDisplayableItemFiles(mergeFileLists(getItemInlineLabelFiles(item), files));
  ...
  if (fileList.length === itemCount && fileList[index]) return [fileList[index]];
  return [];
};

const getLineItemUploadMatchKey = (item = {}, index = 0) =>
  String(getLineItemEntityId(item) || getLineItemSku(item) || getLineItemFnsku(item) || `index:${index}`);
...
const findCreatedLineItemForUpload = (sourceItem = {}, fallbackIndex = 0, createdLineItems = [], usedKeys = new Set()) => {
  ...
  const sourceOrder = getLineItemDisplayOrder(sourceItem);
  ...
  return useMatch(
    findBy((lineItem) => {
      const candidateSku = normalizeItemFileMatchValue(getLineItemSku(lineItem));
      const candidateFnsku = normalizeItemFileMatchValue(getLineItemFnsku(lineItem));
      return sourceSku && sourceFnsku && sourceSku === candidateSku && sourceFnsku === candidateFnsku;
    }) ||
      findBy((lineItem) => sourceSku && sourceSku === normalizeItemFileMatchValue(getLineItemSku(lineItem))) ||
      findBy((lineItem) => sourceFnsku && sourceFnsku === normalizeItemFileMatchValue(getLineItemFnsku(lineItem))) ||
      findBy((lineItem) => sourceProduct && sourceProduct === normalizeItemFileMatchValue(getLineItemProductName(lineItem))) ||
      findBy((lineItem) => sourceOrder !== null && sourceOrder === getLineItemDisplayOrder(lineItem)) ||
...
const normalizeShipment = (shipment) => ({
  ...
  assigned: getAssignedStaffName(shipment) || 'Unassigned',
  assignedStaffId: getAssignedStaffId(shipment),
...
const buildShipmentItems = (items) => {
  ...
  return validItems.map((item) => {
    ...
    const services = [
      ...
      ...(item.needsBundling ? ['Bundling'] : []),
    ]
      .map((service) => normalizeServiceType(String(service).split('/')[0]))
      .filter(Boolean);

    const bundleSize = item.needsBundling ? Number(item.bundleSize || 0) : 0;
    const bundlePayload = isPositiveBundleSize(bundleSize)
      ? {
          bundleSize,
          bundle_size: bundleSize,
          bundle: bundleSize,
          casePack: bundleSize,
          case_pack: bundleSize,
          needsBundling: true,
          needs_bundling: true,
        }
      : item.needsBundling
    ...
    return {
      ...
      fnskuLabelFileName: item.fileName || undefined,
      fnsku_label_file_name: item.fileName || undefined,
        ...
        if (labelFileInputs.length) {
          ...
          const uploadResults = await Promise.allSettled(
            labelFileInputs.map(async ({ file, item, index }) => {
              const savedLineItem = findCreatedLineItemForUpload(item, index, createdLineItems, usedCreatedLineItemKeys);
          ...
          const failedUploads = uploadResults.length - labelUploadCount;

          if (failedUploads) {
            postCreateWarnings.push(`${failedUploads} FNSKU label file(s) could not upload.`);
          }
        }
                                    ...
                                    {unassignedDiscrepancies.map((discrepancy, index) => {
                                      ...
                                      return (
                                        <div key={discrepancy?.id || discrepancy?.uuid || getDiscrepancyLineItemId(discrepancy) || index} className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
                                          ...
                                          <p className="mt-1 text-xs text-amber-800">
                                            Expected {getDiscrepancyExpectedQty(discrepancy, matchedLineItem)}, received {getDiscrepancyReceivedQty(discrepancy, matchedLineItem)}
                                          </p>
                                        </div>
                                      );
                                    })}
