...
const getClientCompanyName = (session = {}) =>
  session?.companyName ||
  ...
  session?.rawUser?.clientName ||
  session?.rawUser?.client_name ||
  session?.rawUser?.client?.companyName ||
  session?.rawUser?.client?.company_name ||
  session?.rawUser?.client?.name ||
  session?.name ||
  "Client";
...
const menuItems = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: "awaiting-fba-labels",
        label: "Awaiting FBA Labels",
        icon: Tags,
...
const ClientSideBar = ({ isOpen = false, onClose = () => {} }) => {
  ...
  const companyName = getClientCompanyName(session || {});
        ...
        {showExpandedSidebar ? (
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="max-w-[150px] truncate text-[10px] uppercase tracking-wider text-blue-100/80">{companyName}</p>
            </div>
          </div>
        ) : (
          ...
          <div key={section.section}>
            ...
            <div className="space-y-0.5">
              {section.items.map((item) => {
                ...
                const itemMode = itemSearchParams.get("mode");
                const isActive =
                  location.pathname === itemPath &&
                  (itemMode
                    ? currentQuery.get("mode") === itemMode
                    : !currentQuery.get("mode"));
                ...
                return (
                  ...
                  >
                    ...
                    {showExpandedSidebar && item.id === "shipments" ? (
                   "" 
                    ) : null}
                  </Link>
                );
...
/*
Note: The above code is for the ClientSideBar component in a React application. It includes functionality for displaying a sidebar with navigation links, handling user sessions, and allowing users to log out. The sidebar can be collapsed or expanded, and it adapts to different screen sizes. The component uses React Router for navigation and localStorage for session management.
*/