...
const ReceiveDialog = ({ item, matchedBy, onConfirm, onCancel }) => {
...
// Scan a product on a shipment being received, confirm the counted units, and save them.
// onReceive(lineItem, receivedQty) must save and refresh; throw to show an error in the dialog.
const ScanReceiveButton = ({ lineItems = [], onReceive, className = '' }) => {
  ...
  return (
    <div className={className}>
      ...
      {scanning ? <BarcodeScanner title="Scan to receive" onDetected={handleDetected} onClose={() => setScanning(false)} /> : null}
      ...
      {choice ? (
        <ScanMatchChooser
          ...
          describe={(item) =>
            `${getScanLineItemName(item)} · FNSKU ${getScanLineItemFnsku(item) || '-'} · ${formatReceivingQuantity(getReceivingLineRemainingQty(item))} still due`
          }
      ...
      {active ? (
        <ReceiveDialog
          ...
          onConfirm={async (quantity, scanNext) => {
            await onReceive(active.item, quantity);
            setNotice('');
            setActive(null);
            if (scanNext) setScanning(true);
          }}
        />
      ) : null}
    </div>
...
export default ScanReceiveButton;
