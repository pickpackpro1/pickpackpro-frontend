...
const resolveDisplayProductName = (...values) => {
  const candidates = values
    .map((value) => String(value || "").trim())
    .filter(Boolean);
  const realName = candidates.find((value) => !isAutoGeneratedProductName(value));
  if (realName) return realName;

  const generatedName = candidates.find(isAutoGeneratedProductName);
  return generatedName ? getAutoGeneratedProductAlias(generatedName) || generatedName : "";
};
...
const getItemName = (item = {}) => {
  const product = getItemProduct(item);
  const productName = resolveDisplayProductName(
    item?.productName,
    item?.product_name,
    item?.name,
    item?.title,
  ...
  if (isGeneratedProductSequenceLabel(productName)) {
    return getConsistentGeneratedProductLabel(item) || getAutoGeneratedProductAlias(productName) || productName;
  }
...
const getItemServices = (item = {}) => {
  ...
  return [...new Set(services.map((service) => String(formatServiceLabel(service) || "").trim()).filter(isDisplayServiceLabel))];
};
...
const getAvailabilityProductName = (item = {}) => {
  const productName = resolveDisplayProductName(getAvailabilityRawProductName(item));

  if (isGeneratedProductSequenceLabel(productName)) {
    const reliableOrder = getAvailabilityReliableSequenceOrder(item);
    return reliableOrder !== null && reliableOrder !== undefined
      ? `p${reliableOrder}`
      : getAutoGeneratedProductAlias(productName) || productName;
  }
...
const ShipmentsStaff = () => {
  ...
  const loadShipmentDetail = async (shipmentId) => {
    try {
      ...
      const boxesRequest = isUuidValue(normalizedShipmentId)
        ? fetch(`${API_BASE_URL}/api/shipments/${encodeURIComponent(normalizedShipmentId)}/boxes`, {
            method: "GET",
            headers: buildHeaders(),
          })
        : Promise.resolve(null);
      const [shipmentResponse, discrepanciesResponse, servicesResponse, boxesResponse, filesResponse] = await Promise.all([
        fetch(`${API_BASE_URL}/api/shipments/${encodeURIComponent(shipmentId)}`, {
          method: "GET",
          headers: buildHeaders(),
        }),
        fetch(`${API_BASE_URL}/api/shipments/${encodeURIComponent(shipmentId)}/discrepancies`, {
          method: "GET",
          headers: buildHeaders(),
        }),
        fetch(`${API_BASE_URL}/api/shipments/${encodeURIComponent(shipmentId)}/services`, {
          method: "GET",
          headers: buildHeaders(),
        }),
        boxesRequest,
        fetch(`${API_BASE_URL}/api/files?entityType=shipment&entityId=${encodeURIComponent(shipmentId)}`, {
          method: "GET",
          headers: buildHeaders(),
        }),
      ]);
      ...
      const discrepanciesPayload = await parseResponse(discrepanciesResponse);
      const servicesPayload = await parseResponse(servicesResponse);
      const filesPayload = await parseResponse(filesResponse);
      let boxesPayload = null;

      if (boxesResponse) {
        try {
          boxesPayload = await parseResponse(boxesResponse);
        } catch {
          boxesPayload = null;
        }
      }

      const shipmentData = extractShipmentDetail(shipmentPayload);
      const shipmentRecordId = getShipmentRecordId(shipmentData);
      let shipmentFiles = toArray(filesPayload?.files || filesPayload?.data || filesPayload);
      if (!shipmentFiles.length && shipmentRecordId && shipmentRecordId !== String(shipmentId || "").trim()) {
        try {
          const shipmentFilesResponse = await fetch(`${API_BASE_URL}/api/files?entityType=shipment&entityId=${encodeURIComponent(shipmentRecordId)}`, {
            method: "GET",
            headers: buildHeaders(),
            cache: "no-store",
          });
          const shipmentFilesPayload = await parseResponse(shipmentFilesResponse);
          shipmentFiles = toArray(shipmentFilesPayload?.files || shipmentFilesPayload?.data || shipmentFilesPayload);
        } catch {
          shipmentFiles = [];
        }
      }
      ...
      const lineItemFileRequests = detailLineItems
        ...
        .filter(Boolean)
        .filter((lineItemId, index, allLineItemIds) => allLineItemIds.indexOf(lineItemId) === index)
        .map((lineItemId) =>
      ...
      const lineItemFiles = lineItemFileResults.flatMap((result) =>
        result.status === "fulfilled"
          ? toArray(result.value?.files || result.value?.data || result.value)
          : []
      );
      ...
      const itemLabelFileResults = await Promise.allSettled(
        itemLabelFileLookups.map(async ({ fileId, item }) => {
          const itemId = getShipmentLineItemId(item) || getLineItemId(item);
          const labelFiles = await fetchFileById(fileId);
          return labelFiles.map((file) =>
            buildItemLabelFile(item, {
              ...file,
      ...
      if (!loadedBoxes.length) {
        const boxLookupIds = getShipmentLookupCandidates(shipmentData, {
          id: shipmentId,
          reference: selectedShipment?.reference,
        })
          .filter(isUuidValue)
        ...
        for (const lookupId of boxLookupIds) {
          try {
      ...
      const boxFileRequests = allKnownBoxes
        .flatMap((box) =>
          getBoxLookupIds(box).map((boxId) => ({
            boxId,
            entityType: isPalletBox(box) ? "pallet" : "box",
          }))
        )
        .filter(({ boxId }) => isUuidValue(boxId))
        .filter(({ boxId }) => Boolean(boxId))
        .filter((lookup, index, lookups) =>
          lookups.findIndex((currentLookup) => currentLookup.boxId === lookup.boxId && currentLookup.entityType === lookup.entityType) === index
        )
        .map(({ boxId, entityType }) =>
          fetch(`${API_BASE_URL}/api/files?entityType=${entityType}&entityId=${encodeURIComponent(boxId)}`, {
            method: "GET",
            headers: buildHeaders(),
            cache: "no-store",
          }).then((response) => parseResponse(response))
      ...
      const fbaLabelFileResults = await Promise.allSettled(
        fbaLabelFileLookups.map(async ({ fileId, boxId, entityType }) => {
          ...
          return files.map((file) => ({
            ...
            fileId: file?.fileId || fileId,
            file_id: file?.file_id || fileId,
              entityType: file?.entityType || entityType,
              entity_type: file?.entity_type || getFileEntityType(file) || entityType,
      ...
      setSubShipmentAvailability(sortAvailabilityRowsForDisplay(subShipmentData.availability || []));
  ...
  const fetchFirstViewPayload = async (lookupCandidates = [], buildUrl) => {
    for (const lookupId of lookupCandidates) {
      try {
        const response = await fetch(buildUrl(lookupId), {
          method: "GET",
          headers: buildHeaders(),
          cache: "no-store",
        });
        ...
        // Shipment endpoints can accept different identifiers depending on the route.
      }
    }

    return { lookupId: "", payload: null };
  };
  ...
  const loadShipmentViewDetail = async (shipment) => {
    ...
    try {
      ...
      const shipmentResult = await fetchFirstViewPayload(
        lookupCandidates,
        (lookupId) => `${API_BASE_URL}/api/shipments/${encodeURIComponent(lookupId)}`
      );
      const shipmentPayload = shipmentResult.payload || {};
      const shipmentDetail = extractShipmentDetail(shipmentPayload);
      const nextLookupCandidates = getShipmentLookupCandidates(shipmentDetail, shipment)
        .concat(lookupCandidates, shipmentResult.lookupId)
        .filter((lookupId, index, allLookupIds) => lookupId && allLookupIds.indexOf(lookupId) === index);
      const boxLookupCandidates = nextLookupCandidates.filter(isUuidValue);

      const [servicesResult, boxesResult, filesResult] = await Promise.all([
        fetchFirstViewPayload(
          nextLookupCandidates,
          (lookupId) => `${API_BASE_URL}/api/shipments/${encodeURIComponent(lookupId)}/services`
        ),
        fetchFirstViewPayload(
          boxLookupCandidates,
          (lookupId) => `${API_BASE_URL}/api/shipments/${encodeURIComponent(lookupId)}/boxes`
        ),
        fetchFirstViewPayload(
          nextLookupCandidates,
          (lookupId) => `${API_BASE_URL}/api/files?entityType=shipment&entityId=${encodeURIComponent(lookupId)}`
        ),
      ]);

      const detailLineItems = sortLineItemsForDisplay(getLineItems(shipmentDetail));
      const currentLineItems = sortLineItemsForDisplay(getLineItems(shipment));
      const shipmentRecordId = getShipmentRecordId(shipmentDetail);
      const nextShipmentDetail = {
        ...shipment,
        ...shipmentDetail,
        id: getShipmentId(shipmentDetail) || getShipmentId(shipment),
        reference: firstPresent(
          shipment?.reference,
          shipmentDetail?.reference,
          shipmentDetail?.shipmentNumber,
          shipmentDetail?.shipment_number,
          shipment?.id
        ),
        status: shipmentDetail?.status || shipment?.status,
        shipment_line_items: detailLineItems.length ? detailLineItems : currentLineItems,
      };
      const loadedBoxes = await enrichBoxesWithItems(
        mergeBoxLists(
          extractBoxes(boxesResult.payload),
          extractBoxes(shipmentPayload),
          extractBoxes(shipmentDetail)
        ),
        detailLineItems.length ? detailLineItems : currentLineItems
      );
      let shipmentFiles = extractFileRecords(filesResult.payload);
      if (!shipmentFiles.length && shipmentRecordId && !nextLookupCandidates.includes(shipmentRecordId)) {
        try {
          const shipmentFilesResponse = await fetch(`${API_BASE_URL}/api/files?entityType=shipment&entityId=${encodeURIComponent(shipmentRecordId)}`, {
            method: "GET",
            headers: buildHeaders(),
          ...
          shipmentFiles = extractFileRecords(await parseResponse(shipmentFilesResponse));
        } catch {
          shipmentFiles = [];
        }
      }

      const viewLineItems = sortLineItemsForDisplay(getLineItems(nextShipmentDetail));
      const lineItemFileRequests = viewLineItems
        .flatMap((item) => [
          getShipmentLineItemId(item),
          getLineItemId(item),
        ])
      ...
      const itemLabelFileLookups = viewLineItems
        .map((item) => ({
          fileId: String(getItemLabelFileId(item) || "").trim(),
          item,
        }))
      ...
      setViewShipmentDetail(nextShipmentDetail);
      setViewShipmentServices(extractServiceTasks(servicesResult.payload));
      setViewShipmentBoxes(loadedBoxes);
      setViewShipmentFiles(mergeFileLists(shipmentFiles, lineItemFiles, itemLabelFiles, boxFiles, fbaLabelFiles, viewLineItems.flatMap((item) => getItemInlineLabelFiles(item))));
    } catch (requestError) {
  ...
  const handleCreateBox = async (isPallet = false) => {
    ...
    try {
      ...
      let boxPayload;
      try {
        boxPayload = await createBoxRequest(createBoxPayload);
      } catch (createError) {
        if (createBoxPayload === payload) throw createError;
        boxPayload = await createBoxRequest(payload);
      }
      const newBox = boxPayload?.box || boxPayload?.data?.box || boxPayload?.data || boxPayload;
      const newBoxId = getBoxRecordId(newBox) || newBox?.id;

      if (!isPallet && newBoxId && allocationPayloadItems.length) {
      ...
      if (!isPallet && newBoxId && allocations.length) {
        ...
        for (const allocation of allocations) {
          ...
          if (createAllocatedQuantity >= allocation.quantity) {
            if (createAllocatedQuantity < allocation.requestedQuantity) {
              allocationWarning += ` ${skuLabel} allocation adjusted to ${formatQuantityValue(createAllocatedQuantity)} units (max available ${formatQuantityValue(allocation.maxQuantity)}).`;
            }
            continue;
          }
          ...
          try {
            ...
            const itemResponse = await fetch(`${API_BASE_URL}/api/boxes/${encodeURIComponent(newBoxId)}/items`, {
              method: "POST",
              headers: buildHeaders(true),
              body: JSON.stringify({
            ...
            await parseResponse(itemResponse);
            ...
            if (refreshedAllocatedQuantity >= allocation.quantity) {
              continue;
            } else if (/max allocatable:\s*0/i.test(String(allocationError?.message || ""))) {
  ...
  const boxSelectionBoxes = activeSubShipmentForBox
    ...
    : boxes;
  const eligiblePalletBoxes = boxSelectionBoxes.filter((box) => {
                                            ...
                                            <div className="flex flex-wrap items-center gap-2">
                                              ...
                                              {insidePallet ? (
                                                <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-blue-700">
                                                  Inside pallet {getBoxPalletId(box)}
                                                </span>
                                              ) : null}
                                            </div>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleUpdateTask({
                                    taskId: serviceId,
                                    status: "DONE",
                                    unitsDone: getServiceUnits(service, serviceLineItem),
                                  })
                                }
                          ...
                          <div className="space-y-3">
                            {viewLineItems.map((item, index) => {
                              ...
                              const itemServices = [
                                ...new Set(
                                  [
                                    ...getLineItemServices(item, standardServiceTasks, itemCount),
                                    ...itemServiceTasks.map(getServiceTaskLabel),
