...
const getLineItemServices = (item, serviceTasks = [], itemCount = 0) => {
  ...
  return mergeServiceTasks(serviceTasks)
    .filter((service) => {
      const serviceLineItemId = String(getServiceTaskLineItemId(service) || "").trim();
      const serviceSku = String(getServiceTaskSku(service) || "").trim().toLowerCase();
      return (
...
const getServiceDisplayStatus = (serviceName, item, serviceTasks) => {
  const serviceKey = normalizeServiceKey(serviceName);
  const itemIds = [getLineItemId(item), getShipmentLineItemId(item)]
    .map((value) => String(value || "").trim())
    .filter(Boolean);
  const itemSku = String(getItemSku(item) || "").trim().toLowerCase();
  const matchedTask = mergeServiceTasks(serviceTasks).find((service) => {
    ...
    return sameLineItem && sameSku && sameType;
  });

  const normalized = String(matchedTask?.status || "").toUpperCase();
  return normalized === "DONE" || normalized === "COMPLETED" ? "Done" : "Pending";
};

const getServiceTaskForLine = (serviceName, item, serviceTasks) => {
  const serviceKey = normalizeServiceKey(serviceName);
  const itemIds = [getLineItemId(item), getShipmentLineItemId(item)]
    .map((value) => String(value || "").trim())
    .filter(Boolean);
...
const ShipmentsStaff = () => {
  ...
  const loadShipmentDetail = async (shipmentId) => {
    try {
      ...
      const lineItemFileRequests = getLineItems(shipmentData)
        .flatMap((item) => [
          getShipmentLineItemId(item),
          getLineItemId(item),
        ])
  ...
  const loadShipmentViewDetail = async (shipment) => {
    ...
    try {
      ...
      const detailLineItems = getLineItems(shipmentDetail);
      const currentLineItems = getLineItems(shipment);
      ...
      const viewLineItems = getLineItems(nextShipmentDetail);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="h-[600px] w-full max-w-2xl overflow-y-auto rounded-xl bg-white shadow-xl">
              {(() => {
                ...
                const viewLineItems = getLineItems(shipmentForView);
                              ...
                              const itemServiceTaskRows = [
                                ...
                                ...itemServices
                                  ...
                                  .map((serviceName, serviceIndex) => {
                                    const matchedTask = getServiceTaskForLine(serviceName, item, standardServiceTasks);
